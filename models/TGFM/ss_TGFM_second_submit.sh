...
to_subroot=$from_dir_sum_stats/TGFM_results
mkdir $to_subroot

to_locus=$to_subroot/locus_$locus_idx
mkdir $to_locus

from_ld_z_dir=$from_dir_sum_stats/TGFM_z_ld
...
do
  ...
  srun --exclusive -n 1 -c 1 --mem-per-cpu 7G Rscript /PATH/TO/ss_TGFM_second_code.R $to_boot $z_file $R_file $gwas_sample_size
