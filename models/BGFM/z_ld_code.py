...
def standardize_effects(effects_folder, num_genes, boot_id, ld_variants_file):
    ...
    for idx_gene in range(num_genes):
        gene_tissue_susie_pmces = np.loadtxt(f"{effects_folder}/bootstrap_susie_g_{(idx_gene+1)}/pos_dist_b_{boot_id}/pmces.csv", delimiter=",", skiprows=1)
...
if __name__ == "__main__":
    raw_eQTL_effects_dir = os.getenv("from_dir_locus")
    num_genes = int(os.getenv("num_genes"))
    ld_variants_file = os.getenv("ld_variants_file")

    to_dir_boot = os.getenv("to_dir_boot")
    boot_id = os.getenv("boot_id")

    window_susie_pmces = standardize_effects(raw_eQTL_effects_dir, num_genes, boot_id, ld_variants_file)
    np.savetxt(f"{to_dir_boot}/window_susie_pmces.csv", window_susie_pmces, delimiter=",", fmt="%.6f")

    full_ld = extract_full_gene_variants_ld(window_susie_pmces, ld_variants_file)
    np.savetxt(f"{to_dir_boot}/full_ld.csv", full_ld, delimiter=",", fmt="%.6f")
    ...
    np.savetxt(f"{to_dir_boot}/z_vector.csv", z_vector, delimiter=",", fmt="%.6f")
