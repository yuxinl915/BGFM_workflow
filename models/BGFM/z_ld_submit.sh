...
from_dir_sampling=$root_dir/BGFM_post_dists_$PVE_X_percent

export from_dir_locus=$from_dir_sampling/locus_$locus_idx

to_dir_subroot=$from_dir_sum_stats/BGFM_z_ld
...
do
  ...
  mkdir $to_dir_boot
  srun --exclusive -n 1 -c 1 --mem-per-cpu 7G python /PATH/TO/z_ld_code.py
